...
- **Do** add `.ph--own` alongside `.ph` for Terk's own photography. The heavy grade exists to make mismatched stock cohere; on a picture carrying Terk's mark on a hard hat it flattens the gold to grey and greys the hi-vis, which defeats the purpose. `.ph--own` lightens the filter to `grayscale(0.26)`, drops the hue wash to 20% and thins the scrim. As stock is replaced with real Terk images, more of the set moves to this grade, and eventually the heavy one retires.
